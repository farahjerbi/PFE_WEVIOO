    ...
    <MDBCard className='ms-5 sms-card'>
      <MDBCardBody>
        <div className='d-flex justify-content-between align-items-center mb-2'>
          {role === Role.ADMIN && (
            <Button
              onClick={() => navigate(CREATE_SMS_TEMPLATE)}
              style={{ width: "20%" }}
              size="small"
              className="mb-2"
            >
            ...
            <tr>
              ...
              {role === Role.USER && (
                <>
                  <th>Save</th>
                  <th>Send Bulk</th>
                  <th>Send Separately</th>
                </>
              )}
            </tr>
          </MDBTableHead>
          <MDBTableBody>
            {currentItems?.filter((template: any) => template.name.toLowerCase().includes(query)).map((template: any) => (
              <tr key={template.id}>
                <td>{template.name}</td>
                  ...
                  <>
                    ...
                    <td>
                      <Tooltip style={{ marginRight: "5px" }} title="Send Bulk" className="color_blue">
                        <Button onClick={() => { navigate(`${SEND_SMS}/${template.id}`) }}>
                    ...
                    <td>
                      <Tooltip style={{ marginRight: "5px" }} title="Send Separately" className="color_baby_bluee">
                        <Button onClick={() => { navigate(`${SEND_SMS_SEPARATELY}/${template.id}`) }}>
